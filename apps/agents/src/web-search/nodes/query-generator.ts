...
const QUERY_GENERATOR_PROMPT = `You're a helpful AI assistant tasked with writing a query to search the web to assist a user (lawyer).
You're provided with a list of messages between a user (lawyer) and an AI assistant.
The most recent message from the user (lawyer) is the one you should update to be a more search engine friendly query.

Try to keep the new query as similar to the message as possible, while still being search engine friendly.

Here is the conversation between the user (lawyer) and the assistant, in order of oldest to newest:
